...
export const Header = ({ cartCount = 0, homeContent }) => {
  ...
  const branding = homeContent?.branding || {};
  const nome_loja = branding.nome_loja || BRANDING_PADRAO.nome_loja;
  const slogan = branding.slogan || BRANDING_PADRAO.slogan;
  const logo_url = branding.logo_url || BRANDING_PADRAO.logo_url;
            ...
            {NAV_ITEMS.map((item, i) => (
              item.to ? (
                <Link
                  key={i}
                  to={item.to}
                  className="relative px-4 py-2 text-xs uppercase tracking-widest text-zinc-400 hover:text-white transition-colors group"
                >
                  {item.label}
                  <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-amber-500 group-hover:w-4 transition-all" />
                </Link>
              ) : (
                <button
                  key={i}
        ...
        {mobileOpen && (
          <div className="md:hidden pb-6 pt-2 border-t border-zinc-800 animate-in slide-in-from-top duration-200">
            ...
            <Link
              to="/catalogo"
              onClick={() => setMobileOpen(false)}
              className="flex items-center justify-center gap-2 bg-amber-500 text-zinc-900 font-bold px-4 py-3 rounded-lg text-sm uppercase tracking-wider hover:bg-amber-400 transition-all mt-4 shadow-lg shadow-amber-500/20"
            >
              <Gem size={16} />
